$(function(){
    ...
    axios.get('http://localhost:20002/border/cityRisk')
    .then((response) => {
    ...
    axios.get('http://localhost:20002/border/borderInfo?city=*')
    .then((response) => {
        if(response.data.status=="200")
        {
            // console.log(response.data.data);
    ...
    var rippleLayerRed = new mapvgl.RippleLayer({
        ...
        onClick: (e) => { // 点击事件
            ...
            //获取当前城市检测出病情的患者轨迹
            axios.defaults.withCredentials = true;
            axios.get('http://localhost:20002/border/targetCityPatient?city='+e.dataItem.geometry.city)
            .then((response) => {
            if(response.data.status=="200")
            {
                // console.log(response.data.data);
